...
2. Run `npm install` at the repo root to install the YAML parser and Playwright library. Run `npx playwright install chromium` to enable the shipped browser computer-use workflow.
3. Check `.env` for non-placeholder credentials required by every model in the selected stack. Never invent, display, or commit keys. Gotchas:
   ...
   - The `fusion`/`fusion5` stacks need `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, and `FIREWORKS_API_KEY`; the trio/legacy pairs need `OPENAI_API_KEY`.
   - `/research-x` and the `x-research` workflow need `XAI_API_KEY` and incur xAI search charges.
...
4. Confirm these load paths exist:
   ...
   - `extensions/fusion-harness/modules/{runtime,child-runner,prompt-library,tui,cmd-readonly,cmd-fusion,cmd-build,cmd-workflows,workflow-registry,x-research,model-stack,agent-layout,collaboration-graph,writer-lease}.ts`
   - `extensions/fusion-harness/prompts/{SYSTEM,USER}_PROMPT_*.md`
   - `extensions/fusion-harness/workflows/*.yaml` and `extensions/fusion-harness/skills/computer-use/SKILL.md`
   - `.pi/fusion-harness/model-stack-{fusion,fusion-5,trio}.yaml`
