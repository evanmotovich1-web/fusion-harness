...
| `/nano-media <brief>` | **nano.media**, the content harness. One real brief (what happened, the real numbers, the artifact you can screenshot) → live "what is winning" research on the Grok seat → every seat drafts three candidates under `prompts/SYSTEM_PROMPT_NANO_MEDIA.md` → a deterministic kill-test drops anything with a number not in the brief, a machine tell, a CTA, a URL, or a fake visual → every seat scores the survivors → the architect rewrites the top three in the operator's voice → a ranked `pack.md`. Nothing is ever posted. |
...
Strong models should not spend turns rediscovering what a child process can measure. The harness now recognizes obvious repository states deterministically, escalates only genuine ambiguity, and requires exact-SHA evidence before parent-owned publication. Publication is not the only irreversible action: lane replacement runs sequentially under the writer lease and captures and revalidates each lane branch SHA, worktree HEAD, and worktree-status hash immediately before forced removal; `/fh-lanes clean` also renders that evidence before deleting anything. Read-only children retain explicit tool allowlists, and publication through the parent is gated by a harness receipt. Write-enabled Fusion children with bash no longer have an enforced delegated-Git command classifier; parent receipts do not intercept their direct Git commands.

**`/fh-repo-state status | refresh [remote]`**

- `status` is strictly local and read-only: exact 40-hex SHAs for HEAD, source, target, and upstream, merge-base, ahead/behind, ancestor relations, dirt (tracked / untracked / unmerged), and freshness — classified into one of seven verdicts. It never fetches and never takes the writer lease.
- `refresh` is the only fetch in this surface: one explicit bounded `git fetch --no-tags --no-recurse-submodules` (30s timeout, no implicit remote choice — multiple remotes require naming one), held under the writer lease, and it reports every changed remote-tracking ref with before/after SHAs before re-collecting the card.

| Verdict | Meaning |
|---|---|
| `proceed` | No publication obstacle; ordinary collaboration is unimpeded. |
| `fast_forward_possible` | Target is an ancestor of source: a non-force push would publish it. |
| `already_integrated` | Source is already contained in target — publication is a no-op. |
| `blocked_dirty` | Working tree has tracked/untracked/unmerged dirt. |
| `blocked_diverged` | Source and target have diverged; only a merge/rebase (parent decision, never automatic) could reconcile. |
| `blocked_stale` | Remote-tracking information is too old to decide publication safely. |
| `needs_decision` | Genuine ambiguity (e.g. missing or ambiguous target): one `select` in UI modes, fail closed without UI. |

**`--publish-to <remote>/<branch>` on `/fh-collaborate`.** Bare branch names map to `refs/remotes/origin/<branch>`, never local `refs/heads/*`. Before any child spawns, the selected remote is refreshed under the writer lease, then the measured card short-circuits: `already_integrated` ends as a no-op summary with zero agents, `blocked_*` refuse with no spawn, no merge, no push, and `needs_decision` asks once via `select` when UI exists, otherwise fails closed. Only after a fully successful collaboration (no blocked/failed/needs_decision tasks, and therefore a write-enabled final integration) does the parent publish, in this order: validate (`git diff --check` plus clean porcelain — a bounded tree gate, not CI), mint an exact-SHA receipt, refresh the target remote, re-collect facts, re-authorize the receipt against a persisted single-use digest ledger, then `git push --no-force <remote> <candidateSha>:refs/heads/<branch>`. An `authorize_noop` pushes nothing. The harness never stashes, merges, rebases, forces, or auto-cleans — divergence and dirty trees are reported, not repaired.

**Exact-SHA receipts.** A receipt binds repository identity (worktree, git common dir, worktree id), the candidate SHA, the observed target SHA, HEAD before and after validation, the validation command with exit code and duration, and an issuance timestamp, sealed with a SHA-256 digest over the canonical serialization. Verification refuses absent, malformed, stale, reused, tampered, wrong-repository, mismatched-SHA, moved-target, failed-validation, dirty, conflicted, detached-HEAD, or diverged evidence. Receipts are single-use (consumed digests are remembered). Only two outcomes ever authorize: an equal/contained no-op, or a non-force fast-forward. An agent-issued claim is never a receipt.

**Fail-closed task outcomes and host recovery.** Every delegated task must end with exactly one metadata line: `FH_TASK_OUTCOME: {"schema_version":1,"status":"completed|no_op|blocked|needs_decision","summary":"…"}`. A malformed read-only report gets one read-only retry with the invalid attempt preserved; malformed write reports are never automatically replayed. `no_op` satisfies dependencies, while `blocked` skips descendants; `needs_decision` asks once, then stops. A blocked, failed, or undecided task skips the child architect's write-enabled integration. The harness persists the failed summary and queues a separate interactive Pi-host recovery turn with the plan and report paths. Pi may independently complete reversible work in the original scope, but cannot treat failed child prose as acceptance or bypass clinical/data/publication gates. This host recovery does not turn the collaboration's `ok: false` into success or publish anything.

**Delegated Git risk vs parent-owned publication.** The child repository guard and its load acknowledgement have been removed. A write-enabled Fusion child with `bash` can attempt direct `git push`, destructive Git operations, or other shell commands if its environment permits them. The parent publication path still requires a harness-minted exact-SHA receipt, non-force push, and writer lease, but those controls do not intercept a child's direct command. Children still run with user Git configuration disabled. The repo-state `status` and collaboration card collection are local; `refresh` and the parent's pre-publication re-check contact the remote. A bash-enabled child can independently initiate network activity.
