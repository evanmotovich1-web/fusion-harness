...
**Fail-closed task outcomes.** Every delegated task must end with exactly one metadata line: `FH_TASK_OUTCOME: {"schema_version":1,"status":"completed|no_op|blocked|needs_decision","summary":"…"}`. Malformed or missing metadata counts as failure. `no_op` and `blocked` outcomes skip the task's descendants in the DAG; `needs_decision` asks once, then stops. A blocked, failed, or undecided task skips the write-enabled final integration.

**Child guard vs parent-owned publication.** Write-enabled children load `child-repo-guard.ts` explicitly after `--no-extensions`, confirm the load with an ack handshake (a missing ack fails that child closed), and run with git aliases disabled. The guard classifies shell commands and blocks delegated `git push`/`fetch`/`pull`/`merge`/`rebase`/`clean`, `reset` beyond bare, destructive branch/tag deletion, worktree-discard forms, `rm` against `.git`, and their compound, wrapped (`sh -c`, `eval`), indirect (`$(…)`, backticks, `xargs`), assignment-prefixed, and `git -c alias.*` forms — failing closed on ambiguity with stable `CHILD-GUARD:*` codes. It is a command classifier, not an OS sandbox: it cannot see shell aliases or writes through other tools, and it does not need to. Publication is exclusively the parent's — harness-minted receipt, non-force, exact-SHA refspec, under the writer lease. Offline by default: neither `status` nor collaboration cards touch the network; only `refresh` and the pre-publication re-check do.
